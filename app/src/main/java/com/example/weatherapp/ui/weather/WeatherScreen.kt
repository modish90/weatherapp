...
import com.example.weatherapp.common.util.DateUtil.toFormattedDate
import com.example.weatherapp.common.util.WeatherUtil.errorMessageMap
...
import com.example.weatherapp.ui.viewstate.WeatherEvent.UpdateSearchWidgetState
import com.example.weatherapp.ui.viewstate.WeatherState
import com.example.weatherapp.ui.weather.components.Animation
import com.example.weatherapp.ui.weather.components.ForecastComponent
import com.example.weatherapp.ui.weather.components.HourlyComponent
import com.example.weatherapp.ui.weather.components.WeatherComponent
...
fun WeatherScreen(
    ...
    val uiState: ScreenState<WeatherState> by viewModel.state.collectAsState()
    ...
    Scaffold(
        topBar = {
            WeatherTopAppBar(
                ...
                onTextChange = { viewModel.dispatchEvent(WeatherEvent.UpdateSearchTextState(it)) },
                onCloseClicked = { viewModel.dispatchEvent(UpdateSearchWidgetState(SearchWidgetState.CLOSED)) },
                onSearchClicked = { viewModel.dispatchEvent(WeatherEvent.GetCityWeatherDetails(it)) },
                onSearchTriggered = {
                    viewModel.dispatchEvent(UpdateSearchWidgetState(SearchWidgetState.OPENED))
                }
            )
        },
...
fun WeatherScreenContent(
    uiState: ScreenState<WeatherState>,
    modifier: Modifier = Modifier,
    viewModel: WeatherViewModel?,
) {
    when (uiState) {
        is ScreenState.Render -> {
            when (uiState.renderState) {
                is WeatherState.LOADING -> {
                    Animation(modifier = Modifier.fillMaxSize(), animation = R.raw.animation_loading)
                }
                is WeatherState.Error -> {
                    WeatherErrorState(uiState = uiState.renderState, viewModel = viewModel)
                }

                is WeatherState.Success -> {
                    WeatherSuccessState(modifier = modifier, uiState = uiState.renderState)
                }
                else -> {}
            }
        }
    ...
    when(uiState) {
        is WeatherState.Error -> {
            Column(
                ...
                Button(onClick = { viewModel?.dispatchEvent(WeatherEvent.GetCityWeatherDetails()) }) {
                    Icon(
                ...
                Text(
                    ...
                    text =  getErrorMessage(uiState.errorWrapper),
...
@Composable
fun getErrorMessage(errorWrapper: ErrorWrapper): String {
    return when {
        errorWrapper.shouldShowFromMap -> {
            stringResource(errorMessageMap[errorWrapper.errorMessage] ?: R.string.error_generic)
        }
        else -> {
